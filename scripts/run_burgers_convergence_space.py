...
from utils_convergence import (
  ...
  load_yaml, 
  make_burgers_exact_pre_shock_solution, 
  make_plot, 
  pairwise_eoc_from_x, 
  parse_solver_stdout, 
  prepare_results_dir, 
...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument("--config",      type=Path, default=Path("./examples/burgers.yaml"), help="Base YAML config.")
  ...
  parser.add_argument("--results-dir", type=Path, default=Path("./output/burgers_convergence_space"), help="Directory where generated configs, outputs, tables, and plots are stored.")
  return parser.parse_args()

def validate_base_config(cfg: dict) -> None:
  """
  Validate that the base config is compatible with the intended Burgers'
  convergence study.
  """
  if cfg["pde"]["type"] != "burgers": 
    raise ValueError(f"This script expects pde.type = 'burgers', got {cfg['pde']['type']!r}.")
...
def compute_final_time(cfg: dict) -> float:
  """
  Compute the final time for the convergence test. For Burgers' equation, we want
  to choose a time before shock formation. For the chosen initial condition, we can
  use a known formula for the shock formation time, but here we just return a
  fixed time that is safely before shock formation for the sin(2*pi*x) IC.
  """
  return FINAL_TIME
...
def main() -> int:
  ...
  case, runs the solver, computes errors against the exact solution, and produces 
  tables and plots of the results.    
  """
  args        = parse_args()
  ...
  exact                    = make_burgers_exact_pre_shock_solution(INITIAL_CONDITION_EXPRESSION, final_time, float(base_cfg["mesh"]["x_left"]), float(base_cfg["mesh"]["x_right"]))
  ...
  for p_order in P_ORDER:
    ...
    print_space_table("DGmini Burgers spatial convergence", rows, INITIAL_CONDITION_EXPRESSION)
  # end for p_order
  ...
  make_plot(
    ...
    title="DGmini Burgers spatial convergence before shock formation", 
